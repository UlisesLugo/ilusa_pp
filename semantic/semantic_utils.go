...
*/
func (sc *SemanticCube) GetReturnType(op Operation, type1, type2 types.CoreType) (types.CoreType, error) {
	...
	result, ok := sc.operations[cube_key]
	if !ok {
...
func GetOperatorHierarchy(op Operation) (int, error) {
	operatorsDict := NewHierarchyDict()
